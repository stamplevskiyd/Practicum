...
Sofa::Sofa(float x, float y, float z, int SofaPrice, char *SofaColor, char *SofaName, int SofaSeatCount,
           char *SofaMaterial, bool SofaFolding) {
    Size[0] = x;
    Size[1] = y;
    Size[2] = z;
    Price = SofaPrice;
...
Cupboard::Cupboard(float x, float y, float z, int CupboardPrice, char *CupboardColor, char *CupboardName,
                   int CupboardShelfCount, int CupboardDoorCount, char *CupboardMaterial) {
    Size[0] = x;
    Size[1] = y;
    Size[2] = z;
    Price = CupboardPrice;
        ...
        if (!Repeat) //если не выпали на ошибку
        {
            ...
            if (!CorrectAnswer) {
                ...
                if (Answer == "Yes") {
                    Repeat = true;
                    CorrectAnswer = true;
                } else if (Answer == "No") //а если ответ ни Yes, ни No, ничего не меняем и идем заново
                {
                    Repeat = false;
                    CorrectAnswer = true;
                } else
